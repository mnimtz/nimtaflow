...
def _ask_settings(settings: dict, provider: Optional[str]) -> dict:
    ...
    if p in ("gemini", "cloud"):
        ...
        # Prefer the local Ollama (M3 describe worker) — keeps it off the API box
        # and fully on-device. Fall back to in-process only if no Ollama configured.
...
async def ask_photo(db: AsyncSession, photo_id: int, question: str,
    ...
    if not (answer or "").strip():
        return {"answer": "", "provider": label, "error": "no_answer"}
