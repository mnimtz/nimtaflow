    ...
    while True:
        try:
            ...
            if e.code in (404, 410, 422):
                # PHOTO-specific, not a server outage: e.g. the claimed photo's
                # thumbnail isn't generated yet / its file is missing ("Kein Bild
                # verfügbar"). Skip THIS photo and immediately try the next claim —
                # no long backoff, don't count it as a server failure.
                print(f"[mac] Foto übersprungen (HTTP {e.code}: {body[:80]})")
                time.sleep(POLL)
            else:
                # 5xx / auth (401/403) / other: real server hiccup → back off.
                fails += 1
