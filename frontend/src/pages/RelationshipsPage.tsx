...
interface GEdge { id: number; from: number; to: number; type: string; label?: string; category: string; directed: boolean }
interface RelType { value: string; label: string; inverse_label: string | null; category: string; category_label: string; directed: boolean }
...
export default function RelationshipsPage() {
  ...
  const [catFilter, setCatFilter] = useState<'all' | 'family' | 'social' | 'professional'>('all')
  ...
  const vEdges = useMemo(() => catFilter === 'all' ? edges : edges.filter(e => e.category === catFilter), [edges, catFilter])
  const selEdges = vEdges.filter(e => e.from === sel || e.to === sel)
            ...
            {vEdges.map(e => {
              ...
              return (
                <line key={e.id} x1={a.x} y1={a.y} x2={b.x} y2={b.y}
                  stroke={CAT_COLOR[e.category] || CAT_COLOR.other} strokeWidth={hi ? 2.5 : 1} strokeOpacity={hi ? 0.9 : 0.25}
                  markerEnd={e.directed ? 'url(#rel-arrow)' : undefined}>
                  <title>{e.label || e.type}</title>
                </line>
              )
            })}
            {nodes.map(n => {
              ...
              {selEdges.length === 0 ? (
                ...
                <ul className="space-y-1.5">
                  {selEdges.map(e => {
                    ...
                    const base = e.label || e.type
                    const label = e.directed ? (e.from === sel ? `${base} →` : `← ${base}`) : base
                    return (
...
function AddRelationModal({ nodes, preselect, onClose, onSaved }: {
  ...
  const grouped = useMemo(() => {
    const g: Record<string, RelType[]> = {}
    for (const t of types) (g[t.category_label] ??= []).push(t)
    return g
  }, [types])
  const cur = types.find(t => t.value === type)
            ...
            <select value={type} onChange={e => setType(e.target.value)} className={sel}>
              {Object.entries(grouped).map(([cat, list]) => (
                <optgroup key={cat} label={cat}>
                  {list.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                </optgroup>
              ))}
            </select>
