        ...
        <form onSubmit={e => { e.preventDefault(); setQuery(draft) }} className="flex items-center gap-2 max-w-2xl mx-auto">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Beschreibe, was du suchst — z. B. „Kinder beim Spielen im Garten“"
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {!query && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-300 text-sm font-medium">Intelligente Suche</p>
            <p className="text-gray-400 text-xs mt-1 max-w-md">
              Durchsucht deine Fotos nach Bedeutung (KI-Beschreibungen & Embeddings), nicht nur nach Dateinamen.
            </p>
        ...
        {error && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <p className="text-sm text-amber-500">{(error as any)?.response?.data?.detail || 'Suche fehlgeschlagen.'}</p>
            <p className="text-xs text-gray-400 mt-1">Embedding-Provider in „Foto-AI“ konfigurieren und Fotos verarbeiten lassen.</p>
          </div>
        )}

        {query && !isFetching && !error && (
          photos.length === 0 ? (
            <p className="text-center text-gray-400 text-sm py-16">Keine passenden Fotos gefunden.</p>
          ) : (
            <>
              <p className="text-xs text-gray-400 mb-3">{photos.length} Treffer für „{query}“ — beste zuerst</p>
              <JustifiedGrid
                photos={photos}
                rowHeight={200}
                gap={4}
