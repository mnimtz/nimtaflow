  ...
  return (
    <div className="fixed inset-0 z-[120] bg-zinc-950/95 backdrop-blur-sm flex flex-col">
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
        <div className="text-white font-semibold">Schnell benennen</div>
        <div className="text-sm text-zinc-400">{named} benannt · {Math.max(0, queue.length - i)} übrig</div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 overflow-auto">
        {isLoading ? (
          ...
          <div className="text-center text-zinc-300">
            ...
            <p className="text-lg font-semibold">Fertig! 🎉</p>
            <p className="text-sm text-zinc-500 mt-1">{named} Personen benannt.</p>
            <button onClick={onClose} className="mt-4 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm">Schließen</button>
          </div>
        ) : (
          <div className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="text-zinc-400 text-sm mb-3">{cur.face_count} Gesichter · {cur.photo_count} Fotos</div>
            ...
            <div className="flex items-center gap-2 mt-4">
              <button onClick={() => name.trim() && nameM.mutate()} disabled={!name.trim() || nameM.isPending}
                ...
                {nameM.isPending ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} Benennen
              </button>
              ...
              <button onClick={() => dissolveM.mutate()} disabled={dissolveM.isPending} title="Keine echte Person / kein Gesicht — Gruppe auflösen"
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-red-300 hover:bg-red-900/30 text-sm">
                <UserX size={15} /> Auflösen
              </button>
            </div>
            <p className="text-[11px] text-zinc-600 mt-3">Enter = benennen · Tab = überspringen</p>
          </div>
        )}
      </div>
    </div>
